    ...
    <section className="relative overflow-hidden px-6 pb-20 pt-24">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.85 }}>
          ...
          <h1 className="mt-5 text-5xl font-semibold leading-tight md:text-7xl">The Planetary Operating System for Land Development Intelligence</h1>
          <p className="mt-6 max-w-2xl text-lg text-[#b5cde6]">{tagline}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/app" className="btn-primary">Launch live workspace</Link>
            <Link href="/experience" className="btn-secondary">Watch cinematic reveal</Link>
          </div>
        ...
        <motion.div className="space-y-4" initial={{ opacity: 0, x: 22 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.1 }}>
          {[
            ...
            <div key={title} className="glass-panel glow-border p-6">
