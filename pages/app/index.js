      ...
      <section className="mx-auto max-w-[1650px] px-4 pb-6 pt-4">
        ...
        <div className="grid gap-3 xl:grid-cols-[260px_minmax(0,1fr)_380px]">
          <aside className="glass-panel p-3">
            ...
            <button className="btn-primary mt-2 w-full text-xs" onClick={generateScenarios} type="button" disabled={loading || boundaryDraft.length < 3}>Generate Scenarios</button>
          </aside>

          <div className="glass-panel overflow-hidden"><LandCommandMap className="h-[620px]" boundary={boundaryDraft} onBoundaryChange={setBoundaryDraft} onParcelMetricsChange={setParcelMetrics} scenario={selectedScenario} /></div>
        ...
        <div className="glass-panel mt-3 p-3 overflow-x-auto">
          <p className="text-xs uppercase tracking-[0.18em] text-cyan-100/80">Comparison table</p>
          <table className="mt-2 min-w-full text-left text-xs"><thead><tr className="border-b border-white/10"><th className="p-2">Scenario</th><th>Plot count</th><th>Revenue</th><th>Cost</th><th>Profit</th><th>Margin</th><th>Efficiency</th><th>Use case</th></tr></thead><tbody>{rankedRows.map((row) => <tr key={row.scenarioId} className="border-b border-white/5"><td className="p-2">{row.name}</td><td>{row.model.totalPlotCount || 0}</td><td>{currency(row.model.estimatedGrossDevelopmentValue)}</td><td>{currency(row.model.estimatedTotalCost)}</td><td>{currency(row.model.projectedProfit)}</td><td>{Number(row.model.marginPercentage || 0).toFixed(1)}%</td><td>{Number(row.model.efficiency || 0).toFixed(1)}%</td><td>{row.model.investmentSummary?.positioning}</td></tr>)}</tbody></table>
        </div>
      </section>
    </PageShell>
  );
