    ...
    <PageShell>
      <section className="mx-auto max-w-6xl px-6 py-12">
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-4">
          <div className="glass-panel p-3">Top Score: {topScenario?.optimizationScore || '-'}</div>
          <div className="glass-panel p-3">Boundary points: {state?.boundary?.length || 0}</div>
          <div className="glass-panel p-3">Comments: {state?.project?.comments?.length || 0}</div>
          <div className="glass-panel p-3">Paid unlocks: {(state?.project?.payments || []).filter((p) => p.status === 'SUCCESS').length}</div>
        </div>

        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          <div className="glass-panel p-4">
            <h2 className="font-semibold">Map & Boundary (editable)</h2>
            <textarea className="mt-3 h-48 w-full rounded bg-[#09111c] p-2 text-xs" value={boundaryText} onChange={(e) => setBoundaryText(e.target.value)} />
            <button className="mt-3 btn-primary" onClick={saveBoundaryAndProfile}>Save boundary + recompute scenarios</button>
          </div>
          <div className="glass-panel p-4">
            <h2 className="font-semibold">Land Profile + AI</h2>
            <input className="mt-3 w-full rounded bg-[#09111c] px-3 py-2" value={zoning} onChange={(e) => setZoning(e.target.value)} placeholder="Zoning" />
            <input className="mt-2 w-full rounded bg-[#09111c] px-3 py-2" value={cost} onChange={(e) => setCost(e.target.value)} placeholder="Construction cost per unit" />
            <button className="mt-3 btn-secondary" onClick={runAI}>Generate board recommendation</button>
        ...
        <div className="mt-8 glass-panel p-4">
          ...
          <button className="mt-3 btn-primary" onClick={initializePayment}>Initialize export unlock payment</button>
          {billing && <pre className="mt-3 overflow-auto rounded bg-[#09111c] p-3 text-xs">{JSON.stringify(billing, null, 2)}</pre>}
        </div>

        <div className="mt-6 flex gap-3">{tabs.map((t)=><Link key={t} className="btn-secondary" href={`/app/projects/${query.projectId}/${t}`}>{t}</Link>)}</div>
      </section>
    </PageShell>
  );
}
