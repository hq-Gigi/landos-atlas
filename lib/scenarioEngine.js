...
function objectiveFactor(objective = 'BALANCED') {
  ...
  if (objective === 'PREMIUM') return 0.92;
  if (objective === 'FAST_DELIVERY') return 0.82;
  return 1;
}
...
export function generateScenarios({ projectId, boundary = [], objective = 'BALANCED', goal = 'BALANCED_PRACTICAL', assumptions = {} }) {
  ...
  return Array.from({ length: 3 }).map((_, i) => {
    ...
    const geometry = buildScenarioGeometry(boundary, { variant: i, objective });
    const frontageEfficiency = Number((Math.min(1, (metrics.frontage * 120) + 0.35 + i * 0.08)).toFixed(2));
    const deliveryMonths = Math.max(6, timelineBase + ((seed + i * 11) % 12) - Math.round(objective === 'FAST_DELIVERY' ? 3 : 0));

    const lotCount = geometry.plots;
    const avgLotSize = geometry.avgLotArea;
    ...
    const weighted = scoreScenario({
      ...
      simplicity: objective === 'FAST_DELIVERY' ? 0.88 : 0.7,
      phasing: objective === 'FAST_DELIVERY' ? 0.9 : 0.72,
      sales: objective === 'PREMIUM' ? 0.82 : 0.68,
