...
export function generateScenarios({ projectId, boundary = [], objective = 'BALANCED', goal = 'BALANCED_PRACTICAL', assumptions = {} }) {
  ...
  return Array.from({ length: 3 }).map((_, i) => {
    ...
    const geometry = buildScenarioGeometry(boundary);
    const feasibility = evaluateFeasibility({ units: yieldUnits, salePrice: Math.round(revenue / yieldUnits), hardCost: Math.round(cost / yieldUnits) });
    const weighted = scoreScenario({
      plots: geometry.plots / 100,
      avgArea: metrics.area,
      efficiency: 1 - geometry.roads / Math.max(geometry.plots, 1),
      reserve: geometry.reserves / Math.max(geometry.plots, 1),
      road: 1 - geometry.roads / Math.max(geometry.plots, 1),
      access: roadEfficiency,
      corner: 0.55 + i * 0.05,
    ...
    return {
      ...
      metrics: { yieldUnits, revenue, cost, margin: Number(margin.toFixed(3)), deliveryMonths, ...feasibility },
