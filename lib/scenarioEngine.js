...
export function generateScenarios({ projectId, boundary = [], objective = 'BALANCED', goal = 'BALANCED_PRACTICAL', assumptions = {}, targetPlotSize = 500, roadWidth = 9 }) {
  ...
  const scenarioProfiles = [
    { name: 'MAX_YIELD', objective: 'MAX_YIELD', targetPlotSizeMultiplier: 0.84, roadWidthDelta: -1.5 },
    { name: 'BALANCED', objective: 'BALANCED', targetPlotSizeMultiplier: 1, roadWidthDelta: 0 },
    { name: 'PREMIUM_LAYOUT', objective: 'PREMIUM_LAYOUT', targetPlotSizeMultiplier: 1.22, roadWidthDelta: 1.8 }
  ];

  return scenarioProfiles.map((profile, i) => {
    const scenarioObjective = profile.objective || objective;
    const weights = objectives[scenarioObjective] || objectives.BALANCED;
    const adjustedPlotSize = Math.max(120, Math.round(targetPlotSize * profile.targetPlotSizeMultiplier));
    const adjustedRoadWidth = Math.max(5, Number((roadWidth + profile.roadWidthDelta).toFixed(1)));
    const drift = 1 + ((seed + i * 97) % 15) / 100;
    const geometry = buildScenarioGeometry(boundary, { variant: i, objective: scenarioObjective, targetPlotSize: adjustedPlotSize, roadWidth: adjustedRoadWidth, minFrontage: assumptions.minFrontage || 12 });
    const frontageEfficiency = Number((Math.min(1, (metrics.frontage * 120) + 0.35 + i * 0.08)).toFixed(2));
    const deliveryMonths = Math.max(6, timelineBase + ((seed + i * 11) % 12) - Math.round((scenarioObjective === 'FAST_DELIVERY' || scenarioObjective === 'FAST_DEVELOPMENT') ? 3 : 0));
    ...
    const revenue = Math.round(saleablePlots * salePricePerPlot * drift * objectiveFactor(scenarioObjective));
    ...
    const weighted = scoreScenario({
      ...
      simplicity: (scenarioObjective === 'FAST_DELIVERY' || scenarioObjective === 'FAST_DEVELOPMENT') ? 0.88 : 0.7,
      phasing: (scenarioObjective === 'FAST_DELIVERY' || scenarioObjective === 'FAST_DEVELOPMENT') ? 0.9 : 0.72,
      sales: (scenarioObjective === 'PREMIUM' || scenarioObjective === 'PREMIUM_LAYOUT') ? 0.82 : 0.68,
    ...
    return {
      id: `${projectId}-${profile.name.toLowerCase()}`,
      name: profile.name,
      ...
      parameters: {
        targetPlotSize: adjustedPlotSize,
        roadWidth: adjustedRoadWidth,
        objective: scenarioObjective
      },
      metrics: {
