...
export async function recomputeFeasibilityForProject(projectId, assumptions = {}) {
  const project = await prisma.project.findUnique({ where: { id: projectId }, include: { layoutOptions: true } });
  if (!project) return [];

  const infraCost = Number(assumptions.infrastructureCost || 0);
  const constructionCostPerM2 = Number(assumptions.constructionCostPerM2 || 0);
  const salePricePerPlot = Number(assumptions.salePricePerPlot || 0);
  const debtRatio = Number(assumptions.debtRatio || 0.5);
  const annualInterestRate = Number(assumptions.annualInterestRate || 0.2);

  const updates = [];
  for (const option of project.layoutOptions) {
    const scenario = option.payload || {};
    const lotCount = Number(scenario?.metrics?.lotCount || scenario?.layout?.lotCount || 0);
    const avgLotSize = Number(scenario?.metrics?.averageLotSize || scenario?.layout?.lotSizes?.average || 0);
    const deliveryMonths = Number(scenario?.metrics?.deliveryMonths || 18);

    const revenue = salePricePerPlot > 0 && lotCount > 0 ? lotCount * salePricePerPlot : Number(scenario?.metrics?.revenue || 0);
    const buildCost = constructionCostPerM2 > 0 && lotCount > 0 ? lotCount * avgLotSize * constructionCostPerM2 : Number(scenario?.metrics?.cost || 0);
    const developmentCost = buildCost + infraCost;
    const financeCost = developmentCost * debtRatio * annualInterestRate * (deliveryMonths / 12);
    const totalCost = developmentCost + financeCost;
    ...
    const payload = {
      scenarioId: scenario.id,
      confidence: scenario.feasibility || 'MEDIUM',
      assumptions: { infraCost, constructionCostPerM2, salePricePerPlot, debtRatio, annualInterestRate },
      model: { revenue, developmentCost, financeCost, totalCost, profit, margin, roi, timelineMonths: deliveryMonths }
    };

    const existing = await prisma.feasibilityReport.findFirst({ where: { projectId, layoutOptionId: option.id }, select: { id: true } });
    if (existing) {
      await prisma.feasibilityReport.update({
        where: { id: existing.id },
        data: { summary: `${scenario.name || 'Scenario'} revenue ${Math.round(revenue)} with ROI ${roi.toFixed(2)}%`, payload }
      });
    } else {
      await prisma.feasibilityReport.create({
        data: { projectId, layoutOptionId: option.id, summary: `${scenario.name || 'Scenario'} revenue ${Math.round(revenue)} with ROI ${roi.toFixed(2)}%`, payload }
      });
    }

    updates.push({ scenarioId: scenario.id, revenue, developmentCost, totalCost, profit, margin: Number(margin.toFixed(3)), roi: Number(roi.toFixed(2)), timeline: deliveryMonths });
  }
  return updates;
}
